...
DEFAULT_REPO_URL = "https://github.com/Automind-Lab/automindlab-stack.git"
DEFAULT_WORKSPACE = Path.home() / ".openclaw" / "workspace" / "automindlab-stack"
DEFAULT_CONTEXT_HOST = Path.home() / "automind-context"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Keep the local OpenClaw workspace aligned with automindlab-stack.")
    parser.add_argument("--repo-url", default=os.environ.get("AUTOMIND_STACK_REPO_URL", DEFAULT_REPO_URL))
    parser.add_argument("--workspace-dir", default=os.environ.get("AUTOMIND_OPENCLAW_WORKSPACE_DIR", str(DEFAULT_WORKSPACE)))
    parser.add_argument("--host-context", default=os.environ.get("AUTOMIND_HOST_CONTEXT_DIR", str(DEFAULT_CONTEXT_HOST)))
    ...
    parser.add_argument("--output", default="workflows/automind-workspace-sync.json")
