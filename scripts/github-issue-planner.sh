...
suggested_targets = ['docs/', 'config/']
checks = ['python3 scripts/validate-ona-skills.py']

if any(word in text for word in ['workflow', 'github action', 'ci', 'automation']):
    scope = 'automation'
    suggested_targets = ['.github/workflows/', 'scripts/']
    checks = ['python3 scripts/validate-ona-skills.py']
if any(word in text for word in ['service', 'contract', 'schema', 'policy', 'worker', 'runtime']):
    scope = 'runtime'
    risk = 'medium'
    suggested_targets = ['services/', 'docs/', 'config/']
    checks = ['python3 scripts/validate-ona-skills.py', 'make diagnostic-ci || true']
if any(word in text for word in ['secret', '.env', 'credential', 'token', 'tenant', 'prod']):
